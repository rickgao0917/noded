 ...
 */
const USER_FRIENDLY_MESSAGES: Record<ShareErrorType, string> = {
  [ShareErrorType.USER_NOT_FOUND]: 
    'The username you entered could not be found. Please check the spelling and try again.',
  
  [ShareErrorType.ALREADY_SHARED]: 
    'This workspace is already shared with that user. You can update their permissions in the share list.',
  
  [ShareErrorType.PERMISSION_DENIED]: 
    'You don\'t have permission to share this workspace. Only the workspace owner can manage sharing.',
  
  [ShareErrorType.INVALID_WORKSPACE]: 
    'This workspace could not be found or may have been deleted. Please refresh the page.',
  
  [ShareErrorType.SHARE_LIMIT_REACHED]: 
    'You\'ve reached the maximum number of shares for this workspace. Please remove some existing shares first.',
  
  [ShareErrorType.NETWORK_ERROR]: 
    'Unable to connect to the server. Please check your internet connection and try again.',
  
  [ShareErrorType.EXPIRED_LINK]: 
    'This share link has expired. Please request a new link from the workspace owner.',
  
  [ShareErrorType.INVALID_INPUT]: 
    'Please enter a valid username. Usernames must be 3-64 characters and contain only letters, numbers, and underscores.',
  
  [ShareErrorType.SELF_SHARE]: 
    'You cannot share a workspace with yourself. Try sharing with another user.',
  
  [ShareErrorType.SERVER_ERROR]: 
 ...
 */
const ERROR_MESSAGE_MAPPING: Record<string, ShareErrorType> = {
...
export class ShareErrorHandler {
   ...
   */
  public handleShareError(error: any): ShareErrorDisplay {
    ...
    const result: ShareErrorDisplay = {
      message,
      type: displayType,
      duration,
      ...(action && { action })
   ...
   */
  public createErrorElement(display: ShareErrorDisplay): HTMLElement {
   ...
   */
  public displayError(container: HTMLElement, error: any): void {
  ...
  private determineErrorType(error: any): ShareErrorType {
  ...
  private getDisplayType(errorType: ShareErrorType): 'error' | 'warning' | 'info' {
    switch (errorType) {
  ...
  private getErrorAction(errorType: ShareErrorType): ShareErrorDisplay['action'] | undefined {
    switch (errorType) {
      case ShareErrorType.NETWORK_ERROR:
        return {
          label: 'Retry',
