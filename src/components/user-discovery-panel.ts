...
export class UserDiscoveryPanel {
  private static instance: UserDiscoveryPanel;
  private readonly logger = new Logger('UserDiscoveryPanel');
  private readonly featureFlags = FeatureFlagService.getInstance();
  private readonly discoveryApi = UserDiscoveryApi.getInstance();
  
  private state: UserDiscoveryState = {
    isLoading: false,
    error: null,
    searchQuery: '',
    results: null,
  ...
  public static getInstance(): UserDiscoveryPanel {
  ...
  private attachEventListeners(): void {
    ...
    // Search input
    const searchInput = this.panelElement.querySelector('#user-search-input') as HTMLInputElement;
  ...
  private async performSearch(query: string): Promise<void> {
    ...
    try {
      ...
      this.logger.logError(error as Error, 'performSearch');
  ...
  private async loadStatistics(): Promise<void> {
    ...
    try {
      ...
      this.logger.logError(error as Error, 'loadStatistics');
    }
   ...
   */
  private renderStatistics(stats: { totalUsers: number; activeUsers: number; totalWorkspaces: number }): void {
    if (!this.panelElement) return;
    ...
    if (activeUsersEl) activeUsersEl.textContent = stats.activeUsers.toString();
    if (totalWorkspacesEl) totalWorkspacesEl.textContent = stats.totalWorkspaces.toString();
  }

  /**
   * Render search results
   */
  private renderSearchResults(users: PublicUserInfo[] | null, isLoading = false, error?: string): void {
    if (!this.panelElement) return;
   ...
   */
  private async selectUser(user: PublicUserInfo): Promise<void> {
    ...
    if (this.panelElement) {
      ...
      this.panelElement.querySelector(`[data-user-id="${user.id}"]`)?.classList.add('selected');
    }
    ...
    try {
      ...
      this.logger.logError(error as Error, 'selectUser');
   ...
   */
  private showUserDetails(user: PublicUserInfo): void {
    if (!this.panelElement) return;

    const userDetails = this.panelElement.querySelector('#user-details') as HTMLElement;
    ...
    if (workspaceCountEl) workspaceCountEl.textContent = user.workspaceCount.toString();
    if (lastLoginEl) {
   ...
   */
  private renderUserWorkspaces(workspaces: PublicWorkspaceInfo[], error?: string): void {
    if (!this.panelElement) return;
   ...
   */
  private setState(updates: Partial<UserDiscoveryState>): void {
