...
import type { ChatCommand, ChatCommandResult } from '../types/chat.types.js';
import { isChatCommand, CHAT_CONSTANTS } from '../types/chat.types.js';

// DOMPurify is loaded globally via script tag in index.html
...
export class ChatInputHandler {
   ...
   */
  public parseCommand(input: string): ChatCommandResult {
    ...
    try {
      ...
      const command = commandStr as ChatCommand;
   ...
   */
  private validateContent(command: ChatCommand, content: string): { isValid: boolean; error?: string } {
   ...
   */
  private getParseErrorMessage(input: string): string {
   ...
   */
  public getCommandHint(input: string): ChatCommand | null {
   ...
   */
  public isValidCommandFormat(input: string): boolean {
