...
export class ConversationManager {
     ...
     */
    async submitPromptForNode(nodeId, promptContent, onStreamingUpdate) {
        const startTime = performance.now();
     ...
     */
    async createChildNodeForPrompt(parentNodeId, promptContent, onStreamingUpdate) {
        const startTime = performance.now();
     ...
     */
    async associateMarkdownWithPreviousPrompt(markdownContent, currentThread) {
        var _a, _b;
        const startTime = performance.now();
                ...
                if (node) {
                    ...
                    if (lastMarkdownBlock) {
                        ...
                        if (blockIndex !== -1) {
                            ...
                            if (previewManager) {
                                try {
                                    await previewManager.setBlockPreviewMode(lastMarkdownBlock.id, // BlockId type
                                    'rendered', 'api');
                                }
                                catch (error) {
