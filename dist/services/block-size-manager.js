...
import { DOMError } from '../types/errors.js';
/**
...
export class BlockSizeManager {
     ...
     */
    preserveBlockSize(blockId, mode) {
        ...
        try {
            ...
            this.logger.logError(error, 'preserveBlockSize', { blockId, mode });
            throw error;
        }
    }
    /**
     ...
     */
    measureAndLockSize(blockElement) {
     ...
     */
    applyHeightConstraint(element, height) {
     ...
     */
    scheduleConstraintRemoval(blockId, element) {
        ...
        if (existingTimeout !== undefined) {
            ...
            this.logger.logBranch('scheduleConstraintRemoval', 'existing_timeout_cleared', true, { blockId });
        }
        // Schedule new timeout
        const timeoutId = window.setTimeout(() => {
            this.logger.logBranch('scheduleConstraintRemoval', 'removing_constraints', true, { blockId });
    ...
    getBlockElement(blockId) {
        ...
        if (!element) {
            throw new DOMError(`Block element not found: ${blockId}`, 'ELEMENT_NOT_FOUND', 'Unable to find block element', {
