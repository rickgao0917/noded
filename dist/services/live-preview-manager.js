...
export class LivePreviewManager {
     ...
     */
    enablePreview(blockId, mode = 'split', correlationId) {
        ...
        try {
            ...
            blockElement.classList.remove('preview-mode-split', 'preview-mode-preview_only', 'preview-mode-edit_only', 'preview-mode-tabbed');
            blockElement.classList.add(`preview-mode-${mode}`);
            // Create preview container if needed
            if (mode !== 'edit_only') {
                this.createPreviewContainer(blockId, blockElement);
            }
            ...
            this.logger.logError(error, 'enablePreview', {
                blockId,
                mode,
                correlationId
            });
     ...
     */
    disablePreview(blockId, correlationId) {
        ...
        try {
            ...
            this.logger.logError(error, 'disablePreview', {
                blockId,
                correlationId
            });
        }
     ...
     */
    changePreviewMode(blockId, mode, correlationId) {
        ...
        try {
            ...
            // Update state
            this.activeBlocks.set(blockId, Object.assign(Object.assign({}, state), { mode, lastUpdate: new Date() }));
            // Update DOM classes
            const blockElement = document.querySelector(`[data-block-id="${blockId}"]`);
            if (blockElement) {
                blockElement.classList.remove('preview-mode-split', 'preview-mode-preview_only', 'preview-mode-edit_only', 'preview-mode-tabbed');
            ...
            this.logger.logError(error, 'changePreviewMode', {
                blockId,
                mode,
                correlationId
            });
     ...
     */
    createPreviewContainer(blockId, blockElement) {
        ...
        try {
            ...
            this.logger.logError(error, 'createPreviewContainer', { blockId });
            throw error;
        }
    }
    /**
     ...
     */
    setupScrollSync(blockId, blockElement) {
        ...
        try {
            ...
            this.logger.logError(error, 'setupScrollSync', { blockId });
        }
    }
    /**
    ...
    queueRender(task) {
        ...
        try {
            ...
            this.logger.logError(error, 'queueRender', {
                blockId: task.blockId
            });
        }
    }
