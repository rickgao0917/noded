...
import { isGraphNode } from '../utils/type-guards.js';
/**
...
export class NodeBranchingService {
     ...
     */
    shouldCreateBranch(blockType, editSource) {
        ...
        this.logger.logBranch('shouldCreateBranch', shouldBranch ? 'should_branch' : 'should_not_branch', shouldBranch, { blockType, editSource });
     ...
     */
    copyNodeWithoutChildren(sourceNodeId) {
        this.logger.logFunctionEntry('copyNodeWithoutChildren', { sourceNodeId });
        const sourceNode = this.nodes.get(sourceNodeId);
        if (!sourceNode) {
            throw new ValidationError('sourceNodeId', sourceNodeId, 'existing node', {
        ...
        // Deep copy blocks
        const copiedBlocks = sourceNode.blocks.map((block, index) => {
            const newBlockId = `block-${timestamp}-${Math.random().toString(36).substring(2, 9)}`;
            return Object.assign(Object.assign({}, block), { id: newBlockId, content: block.content, position: index // Ensure position is set
             });
        });
        // Create new node without children
        const newNode = {
            id: newNodeId,
            position: Object.assign({}, sourceNode.position), // Copy position
            blocks: copiedBlocks,
     ...
     */
    recordBranchMetadata(nodeId, metadata) {
