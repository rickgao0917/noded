...
export class GraphSynchronizer {
     ...
     */
    async syncNewChildNode(parentNodeId, newNodeId) {
     ...
     */
    async syncNodeBlockAddition(nodeId, blockId) {
        const startTime = performance.now();
     ...
     */
    async syncResponseBlock(nodeId, blockId, content) {
    ...
    async handleNodeDeletion(deletedNodeId) {
     ...
     */
    // @ts-ignore - Will be used when GraphEditor events are implemented
    async handleNodeModification(nodeId, blockId) {
